...
namespace nexus {
    ...
    hole_pitch_      (1   * mm)
  {
    ...
    G4GenericMessenger::Command&
    tpc_height_cmd = msg_->DeclareProperty("tpc_height", tpc_height_, "TPC height");
    tpc_height_cmd.SetUnitCategory("Length");
    tpc_height_cmd.SetParameterName("tpc_height", false);
    tpc_height_cmd.SetRange("tpc_height>0.");
  ...
  void LHM::Construct()
  {
    // LAB
    // ------------------------------------------
    G4double         lab_size  = std::max(tpc_radius_, tpc_height_) * 3;
    ...
    // LXe
    // ------------------------------------------
    G4double         lxe_length = tpc_height_ + gem_thickness_ + interface_height_;
